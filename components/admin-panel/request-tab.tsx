...
function RequestTab() {
  const [pendingRequests, setPendingRequests] = useState<PendingSalonRequest[]>(
    []
  );
  const [error, setError] = useState<string | null>(null);

  const [isLoading, setIsLoading] = useState(false);
  const [requestHandling, setRequestHandling] = useState(false);
  ...
  const approveRequest = async (salonId: string) => {
    setRequestHandling(true);
    const result = await approveSalonRequest(salonId);

    if (result.success) {
      setPendingRequests((prev) =>
        prev.filter((request) => request.id !== salonId)
      );
      setSelectedRequestId((prev) =>
        prev === salonId ? pendingRequests[0]?.id || "" : prev
      );

      toast.success("Salon request approved.");
    } else {
      toast.error("Failed to approve salon request.");
    }
    setRequestHandling(false);
  };

  const rejectRequest = async (salonId: string, reason: string) => {
    setRequestHandling(true);
  ...
  useEffect(() => {
    const getRequests = async () => {
      setIsLoading(true);
      try {
        const data = await getPendingSalonRequests();
          ...
          <>
            <div className="mb-6 flex flex-col gap-4 border-b pb-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  onClick={() =>
                    rejectRequest(selectedRequest.id, "Not specified")
                  }
                  disabled={requestHandling}
              ...
              <section>
                ...
                <div className="grid grid-cols-1 gap-x-8 gap-y-4 text-sm md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {selectedRequest.user?.email || "No owner email provided"}
                    </p>
                  </div>
                </div>
              </section>
