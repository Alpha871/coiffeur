...
import { InviteInformation } from "./_component/invite-information";

export default async function InvitationPage({
  params,
}: PageProps<"/invites/[id]">) {
  const session = await auth.api.getSession({ headers: await headers() });
  if (session == null) return redirect("/authentication");
  ...
  return (
    <div className="container mx-auto my-6 max-w-2xl px-4">
      <Card>
        <CardHeader>
          <CardTitle>Barber shop Invitation</CardTitle>
          <CardDescription>
            You have been invited to join the {invitation.organizationName}{" "}
            barber shop as a {invitation.role}.
          </CardDescription>
        </CardHeader>
        <CardContent>
