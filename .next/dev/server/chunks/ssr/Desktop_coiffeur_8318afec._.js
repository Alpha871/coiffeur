...
(()=>{
    const e = new Error("Cannot find module '@/oop/infrastructure/user-action'");
...
function CreateInviteButton({ type = "default", setPendingInvites, setActiveId }) {
    ...
    async function handleCreateInvite(data) {
        try {
            const result = await sendInvitation(data.email, data.role, id, activeOrganization?.id);
            if (!result) {
...
salonServices }) {
    ...
    async function handleSaveInfo(values) {
        ...
        try {
            const result = await updateUserInfo(active.user.id, salonId, changes);
            if (!result) return;
    ...
    async function handleSaveSpecialties(values) {
        if (!active) return;
        try {
            const result = await assignMemberSpecialties(active.id, salonId, values.skills);
            console.log({
                result
            });
            // setStaff((prev) =>
...
//# sourceMappingURL=Desktop_coiffeur_8318afec._.js.map